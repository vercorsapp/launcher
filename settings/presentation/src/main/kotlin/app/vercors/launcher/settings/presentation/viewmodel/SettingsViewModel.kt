...
class SettingsViewModel(
    ...
    override fun SettingsUiState.reduce(intent: SettingsUiIntent) =
        when (intent) {
            ...
            is ConfigUpdated -> SettingsUiState.Loaded(config = intent.config)
        }
    ...
    private fun SettingsUiState.toggleSection(section: HomeSectionConfig): SettingsUiState =
        if (this is SettingsUiState.Loaded)
            updateConfig(
                ...
                if (section in config.home.sections) config.home.sections - section
                else (config.home.sections + section).sorted()
            )
        else this
